...
public class ItemForceAxe extends ItemAxe {
    ...
    static void attatchInformation(ItemStack stack, List toolTip) {
        if (stack.getCapability(DCRCapabilityHandler.CAPABILITY_TOOLMOD, null) != null) {
            if(stack.getCapability(DCRCapabilityHandler.CAPABILITY_TOOLMOD, null).getSpeedLevel() > 0)
                toolTip.add("Speed " + stack.getCapability(DCRCapabilityHandler.CAPABILITY_TOOLMOD, null).getSpeedLevel());
        }
    }
}
