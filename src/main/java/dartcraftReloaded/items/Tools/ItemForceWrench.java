    ...
    public EnumActionResult onItemUseFirst(EntityPlayer player, World world, BlockPos pos, EnumFacing side, float hitX, float hitY, float hitZ, EnumHand hand) {
        if (player.getHeldItem(hand).getItem() instanceof ItemForceWrench) {
            if (world.getTileEntity(pos) instanceof TileEntity) {
                if (world.isRemote) {
                    world.playSound(player, pos, DCRSoundHandler.SPARKLE, SoundCategory.PLAYERS, 1.0f, 1.0f);
                    return EnumActionResult.SUCCESS;
                } else {
                    ItemStack te = new ItemStack(ModItems.itemTE);
                    IItemTE cap = te.getCapability(DCRCapabilityHandler.CAPABILITY_TE, null);
